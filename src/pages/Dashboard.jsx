...
import { Clock, CheckSquare, Trophy, Play, Pause, RotateCcw, Plus, Trash2, Calendar as CalendarIcon } from "lucide-react";
...
const Dashboard = () => {
  ...
  const handleSessionEnd = () => {
    ...
    if (!isBreak && currentTask) {
      setTodos(todos.map(todo => 
        todo.id === currentTask ? { ...todo, completedSessions: todo.completedSessions + 1 } : todo
      ));
    }
    setIsBreak(!isBreak);
  ...
  const stats = [
    { 
      ...
      value: `${Math.floor(todos.reduce((acc, t) => acc + t.completedSessions, 0) * workTime / 60)}h ${(todos.reduce((acc, t) => acc + t.completedSessions, 0) * workTime) % 60}m`,
    ...
    { 
      ...
      value: `${todos.filter(t => t.completedSessions >= 4).length}/${todos.length}`,
    ...
    <div className="min-h-screen p-8 space-y-8 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10">
        {stats.map((item, index) => (
          ...
          >
            <div className="flex items-center">
              <div className={`p-3 rounded-lg bg-gradient-to-br ${item.bg} border border-gray-800/30`}>
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-purple-900/20 to-purple-900/5 rounded-2xl border border-gray-800/50 backdrop-blur-lg p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group"
        >
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-medium text-gray-400">
                {isBreak ? "Break Time" : currentTask ? "Working On" : "Focus Timer"}
              </h3>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Break (min)</label>
                <input
                  type="number"
                  value={breakTime}
                  onChange={(e) => {
      ...
      >
        ...
        <form onSubmit={addTodo} className="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
          <input
            type="text"
            value={newTodo}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`${todo.completedSessions >= 4 ? 'line-through text-gray-500' : 'text-gray-200'}`}>
                      {todo.title}
                    </span>
              ...
              {todo.completedSessions < 4 && (
                <div className="mt-2 h-2 bg-gray-700/30 rounded-full">
                  <div
                    ...
                    style={{ width: `${getProgress(todo.completedSessions)}%` }}
                  />
                </div>
              )}
            </motion.div>
      ...
      <div className="absolute top-0 left-1/3 w-96 h-96 bg-blue-900/20 rounded-full blur-[100px] pointer-events-none" />
    </div>
  );
};
